...
import time
import os
from my_custom_msgs.msg import Position, Speed, Output, CurrentState, CollectCurrentState
import threading
import csv
import numpy as np
import casadi as ca
...
class CollectDataNode(Node):
    def __init__(self):
        ...
        self.collect_current_state_raw = CollectCurrentState()
        self.pcmd = Output()
        ...
        self.previous_time_raw = None
        self.previous_x_raw = None
        self.previous_y_raw = None
        self.previous_z_raw = None
        self.previous_roll_raw = None
        self.previous_pitch_raw = None
        self.previous_yaw_raw = None

        self.window_size = 100
        self.cutoff_freq_x = 0.5
        ...
        self.data_dir = os.path.join(os.path.expanduser("~"), 'anafi_simulation', 'data', 'real_drone_state_function','in_time_fourier_filter')
        os.makedirs(self.data_dir, exist_ok=True) 
        self.data_dir_raw = os.path.join(os.path.expanduser("~"), 'anafi_simulation', 'data', 'real_drone_state_function','in_time_fourier_filter_raw')
    ...
    def fourier_transform(self, new_position, axis):
        position_window_np = []
        delta_t = 1 / self.freq
        ...
        nyquist = 0.5 * self.freq  # Nyquist frequency
        normal_cutoff = cutoff_freq / nyquist  # Normalized cutoff frequency

    # Apply Butterworth filter
        b, a = butter(4, normal_cutoff, btype='low', analog=False)
        smoothed_position = filtfilt(b, a, position_window_np)

        # print(smoothed_position[-1])
        # print(new_position)
        
        return smoothed_position[-1]
    
    def collect_data_fourier_filter(self):
        ...
        for axis in positions.keys():
            ...
            update_positions[axis] = self.fourier_transform(positions[axis], axis)
    ...
    def collect_data_raw(self):
        ...
        previous_positions = {"x": self.previous_x_raw, "y": self.previous_y_raw, "z": self.previous_z_raw,
                              "yaw": self.previous_yaw_raw, "roll":self.previous_roll_raw, "pitch":self.previous_pitch_raw}
        
        if self.previous_time_raw is not None:
            for axis in speeds.keys():
                ...
                if previous_position is not None:
                    ...
                    delta_time = current_time - self.previous_time_raw
                    speeds[axis] = delta_position / delta_time

        self.previous_x_raw = positions['x']
        self.previous_y_raw = positions['y']
        self.previous_z_raw = positions['z']
        self.previous_yaw_raw = positions['yaw']
        self.previous_roll_raw = positions['roll']
        self.previous_pitch_raw = positions['pitch']
        self.previous_time_raw = current_time

        self.collect_current_state_raw.current_state.position.x = positions['x']
        self.collect_current_state_raw.current_state.position.y = positions['y']
        self.collect_current_state_raw.current_state.position.z = positions['z']
        self.collect_current_state_raw.current_state.position.yaw = positions['yaw']
        self.collect_current_state_raw.current_state.position.pitch = positions['pitch']
        self.collect_current_state_raw.current_state.position.roll = positions['roll']
        self.collect_current_state_raw.current_state.speed.x_speed = speeds['x']
        self.collect_current_state_raw.current_state.speed.y_speed = speeds['y']
        self.collect_current_state_raw.current_state.speed.z_speed = speeds['z']
        self.collect_current_state_raw.current_state.speed.yaw_speed = speeds['yaw']
        self.collect_current_state_raw.current_state.speed.pitch_speed = speeds['pitch']
        self.collect_current_state_raw.current_state.speed.roll_speed = speeds['roll']   


    def calculate_speed_callback(self):
        self.collect_data_fourier_filter()
        self.collect_data_raw()
    ...
    def handle_user_input(self):
        while rclpy.ok():
            try:
                user_input = input('Enter [1:x or 2:y or 3:z or 4:yaw]: ')
                if user_input in ['1', '2', '3', '4']:
                    ...
                    if start_command == "5":                    
                        ...
                        self.csv_file_raw = os.path.join(self.data_dir_raw, f'state_data_{self.test_axis}.csv')
    ...
    def write_csv_header(self):
        ...
        with open(self.csv_file_raw, mode='w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(['elapsed_time',
                             'x', 'y', 'z', 'pitch', 'roll', 'yaw',
                             'x_speed', 'y_speed', 'z_speed', 'pitch_speed', 'roll_speed', 'yaw_speed',
    ...
    def save_data_callback(self):
        if self.write_data_flag:
            ...
            data_raw = [formatted_time_stamp,
                    self.collect_current_state_raw.current_state.position.x, 
                    self.collect_current_state_raw.current_state.position.y,
                    self.collect_current_state_raw.current_state.position.z,
                    self.collect_current_state_raw.current_state.position.yaw,
                    self.collect_current_state_raw.current_state.position.pitch,
                    self.collect_current_state_raw.current_state.position.roll,
                    self.collect_current_state_raw.current_state.speed.x_speed,
                    self.collect_current_state_raw.current_state.speed.y_speed,
                    self.collect_current_state_raw.current_state.speed.z_speed,
                    self.collect_current_state_raw.current_state.speed.yaw_speed,
                    self.collect_current_state_raw.current_state.speed.pitch_speed,
                    self.collect_current_state_raw.current_state.speed.roll_speed,
                    self.control_output.control_x, 
                    self.control_output.control_y, 
                    self.control_output.control_z, 
                    self.control_output.control_yaw]
            ...
            with open(self.csv_file_raw, mode='a', newline='') as file:
                writer = csv.writer(file)
