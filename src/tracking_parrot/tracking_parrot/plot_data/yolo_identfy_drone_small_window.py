    ...
    for result in results:
        if result.boxes:
            for box in result.boxes:
                ...
                x1_resize = max(x1-resize,0)
                y1_resize = max(y1-resize,0)
                x2_resize = min(x2+resize, 1280)
                y2_resize = min(y2+resize, 720)
                ...
                if label.lower() == 'drone' and confidence >= 0.5:  # Replace with the actual drone class name
                    ...
                    resized_box = (x1_resize, y1_resize, x2_resize, y2_resize)
                    return resized_box
                else:
                    print(f"Other object detected: {label} with confidence {confidence:.2f}")

    print("No drone detected with sufficient confidence.")
    return None
    # Display the output
...
def detect_drone_3d(image_path, index, save_dir_3d, resized_box):
    x1_resize, y1_resize, x2_resize, y2_resize = resized_box
    ...
    cropped_frame = frame[y1_resize:y2_resize, x1_resize:x2_resize]
    msg = bridge.cv2_to_imgmsg(cropped_frame, "bgr8")
    ...
    filtered_scores = np.where(scores > 0.7)[0]
...
if __name__ == "__main__":
    ...
    for i in range (0, 44):
        index = i
        resized_box = detect_drone(f'/home/yousa/anafi_simulation/src/tracking_parrot/tracking_parrot/parrot_fig_test/{i}.jpg', index, save_dir)
        if resized_box is not None:
            detect_drone_3d(f'/home/yousa/anafi_simulation/src/tracking_parrot/tracking_parrot/parrot_fig_test/{i}.jpg', index, save_dir_3d, resized_box)