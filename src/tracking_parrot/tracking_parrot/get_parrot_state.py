...
class AnafiState(Node):
    def __init__(self):
        ...
        self.publisher_parrot_state = self.create_publisher(CurrentState, '/parrot_state_with_speed',10)
    ...
    def subscribe_drone_state_callback(self, msg):
        ...
        def process_transform_anafi(transform: TransformStamped):
            ...
            self.parrot_state.position.yaw = yaw
            ...
            if self.previous_time is not None:
                if current_time - self.previous_time >= 0.1:
                    ...
                    positions = {"x":self.parrot_state.position.x, "y":self.parrot_state.position.y, "z":self.parrot_state.position.z,
                                "yaw": self.parrot_state.position.yaw, "roll":self.parrot_state.position.roll, "pitch":self.parrot_state.position.pitch}
                    ...
                    self.parrot_state.speed.x_speed_world = speeds["x"]
                    self.parrot_state.speed.y_speed_world = speeds["y"]
                    self.parrot_state.speed.z_speed = speeds["z"]
                    self.parrot_state.speed.yaw_speed = speeds["yaw"]
                    # self.drone_state.speed.roll_speed = speeds["roll"]
                    # self.drone_state.speed.pitch_speed = speeds["pitch"]

                    self.previous_x = self.parrot_state.position.x
                    self.previous_y = self.parrot_state.position.y
                    self.previous_z = self.parrot_state.position.z
                    self.previous_yaw = self.parrot_state.position.yaw
                    self.previous_roll = self.parrot_state.position.roll
                    self.previous_pitch = self.parrot_state.position.pitch
        ...
        for transform in msg.transforms:
            if transform.child_frame_id == self.target_frame_parrot:
