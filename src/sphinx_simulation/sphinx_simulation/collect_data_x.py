...
class CollectDataNode(Node):
    def __init__(self):
        ...
        self.csv_file = os.path.join(self.data_dir, 'pitch2x_speed.csv')
    ...
    def process(self):
        if self.running:
            try:
                ...
                for _ in range(5):
                    self.collect_and_publish_pcmd(duration=1, interval=1 / self.freq, output_file='drone_state_data_pcmd.csv',
                                                  x_input=20, y_input=0, z_input=0, yaw_input=0)
                    self.collect_and_publish_pcmd(duration=10, interval=1 / self.freq, output_file='drone_state_data_pcmd.csv',
                                                  x_input=0, y_input=0, z_input=0, yaw_input=0)
